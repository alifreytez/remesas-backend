...
import { BaseRepository, type GetAllOptions, type WhereCondition, WhereOperators, type QueryResult } from '@bases/repository.base.js';
...
export class SequelizeRepositoryBase<T = any, ID extends Identifier = string> extends BaseRepository<T, ID, ModelWithAssociate> {
    ...
    async bulkCreate(data: Partial<T>[], operationOptions?: BulkCreateOptions): Promise<T[]> {
        return this.executeWithLogging('bulkCreate', async () => {
            try {
                ...
                const { ignoreDuplicates = false, ignoreFields = [], updateOnDuplicate, ...restOptions } = operationOptions || {};
                ...
                if (ignoreDuplicates && ignoreFields.length > 0) {
                    const filters = data.map((item) => {
                        ...
                        ignoreFields.forEach((field) => {
                            ...
                            if (fieldValue == null) throw new Error(`Campo '${String(field)}' es requerido para la verificación de duplicado`);
                    ...
                    const fingerprints = new Set(existing.map((row: any) => ignoreFields.map((f) => row[f as string]).join('|')));

                    const toInsert = data.filter((item) => {
                        const itemFingerprint = ignoreFields.map((f) => (item as Record<string, unknown>)[f as string]).join('|');
    ...
    async update(criteria: ID | ID[] | WhereCondition, data: Partial<T>, operationOptions?: OperationOptions): Promise<number> {
    ...
    async restore(criteria: ID | ID[] | WhereCondition, operationOptions?: OperationOptions): Promise<number> {
        return this.executeWithLogging('restore', async () => {
            try {
                ...
                // Contar cuántos registros están realmente eliminados antes de restaurar
                const count = await this._model.count({
                    where: { ...where, deletedAt: { [Op.ne]: null } },
                    paranoid: false,
                    transaction: operationOptions?.transaction,
                });
    ...
    async transaction<T = any>(callback?: (t: Transaction) => Promise<T>, isolationLevel: keyof typeof Transaction.ISOLATION_LEVELS = 'READ_COMMITTED'): Promise<T | Transaction> {
    ...
    protected applyOperationOptions(targetOptions: any, operationOptions?: OperationOptions): void {
        ...
        const { transaction, lock, skipLocked, paranoid, returning, plain, mapToModel, raw, ...restOptions } = operationOptions;
