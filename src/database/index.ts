...
class DatabaseManager {
    ...
    async initializeConnectors(): Promise<void> {
        ...
        const promises = DatabaseConfig.loadAll()
            ...
            .map(async (config) => {
                Logger.natural(`Trying to connect to '${config.id}' database as '${config.type}' type on (${config.host}:${config.port}:${config.username})`);
        ...
        if (typeof import.meta.env !== 'undefined') {
            ...
            for (const config of dbConfig) {
                ...
                for (const pathRepo in repoModules) {
                    ...
                    if (config.availableTestingEnv) this.repositories.set(`${config.id}-test.${repoRawName}`, repoDefinition);
                    loadedCount++;
                }

                if (loadedCount > 0) {
                    Logger.natural(ANSI.success(`[+] Repositories for '${config.id}' database as '${config.type}' type loaded`));
                }
            }
        } else {
            ...
            for (const config of dbConfig) {
                ...
                for (const repoName of repoNames) {
                    ...
                    if (!repoDefinition.default || typeof repoDefinition.default !== 'object') throw new Error(`Repository ${repoName} does not export a valid instance`);
    ...
    repository(connectorName: string, repoName: string): BaseRepository<any, any> {
        ...
        if (context?.isTestingRequest && this.repositories.has(`${connectorName}-test.${repoName}`)) key = `${connectorName}-test.${repoName}`;
    ...
    list(connectorName: string | null = null): Array<{ connectorName: string; repoName: string }> {
        return Array.from(this.repositories.keys()).reduce(
            (acu: Array<{ connectorName: string; repoName: string }>, cur: string): Array<{ connectorName: string; repoName: string }> => {
