...
export class App {
    ...
    async start(): Promise<http.Server> {
        ...
        return new Promise((resolve, reject) => {
            ...
            server.listen(this.appConfig.port, this.appConfig.host, () => {
                Logger.natural(ANSI.info(`Server running on ${ANSI.link(this.appConfig.apiBaseUrl)}${ANSI.getCode('reset')}`), { sepStart: true });
    ...
    private async setupRoutes(): Promise<void> {
                ...
                ...(this.appConfig.appEnv == 'development'
                    ? {
                          development: {
                              ...
                              server_time:
                                  new Date().toLocaleString('en-US', { timeZoneName: 'short' }) + ' | ' + new Date().toLocaleString('es-VE', { timeZoneName: 'short', timeZone: 'America/Caracas' }),
            ...
            if (typeof import.meta.env !== 'undefined') {
                ...
                for (const pathRoute in routeModules) {
                    ...
                    try {
                        ...
                        if (!routeHandler || typeof routeHandler !== 'function') throw new Error(`Module ${moduleName} does not export a valid router`);

                        router.use(`/${moduleName}`, routeHandler);
                        Logger.natural(`Loaded: ${ANSI.link(`${this.appConfig.apiBaseUrl}${apiPrefix}/${moduleName}`)}${ANSI.getCode('reset')}`);
                        ++routesCounter;
                    } catch (error: any) {
                ...
                for (const moduleName of moduleNames) {
                    ...
                    const possibleRoutes = [path.join(modulesPath, moduleName, '_.route.js'), path.join(modulesPath, moduleName, '_.route.ts')];
        ...
        if (this.appConfig.enableDocs) {
            try {
                ...
                this.app.use(this.appConfig.docs?.path || '', swaggerUi.serve, (req: Request, res: Response, next: NextFunction) => {
                    const interfaceIp = req.headers.host;
                ...
                Logger.natural(ANSI.success(`Swagger UI loaded: ${ANSI.link(`${this.appConfig.apiBaseUrl}${this.appConfig.docs?.path}`)}${ANSI.getCode('reset')}`));
            } catch (error: any) {
    ...
    private setupErrorHandling(): void {
        ...
        this.app.use((err: any, req: Request, res: Response, next: NextFunction) => {
            ...
            Logger.natural(`${ANSI.error('Error Devuelto:')} ${ANSI.getCode('error')}${error.traceId}${ANSI.getCode('reset')}`, {
                sepEnd: true,
            });
