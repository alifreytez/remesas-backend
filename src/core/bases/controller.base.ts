...
export abstract class ControllerBase {
    ...
    private get currentRequest(): Request | null {
        return executionContextStore.getStore()?.currentRequest || null;
    }
    private get currentResponse(): Response | null {
        return executionContextStore.getStore()?.currentResponse || null;
    }
    private get currentNext(): NextFunction | null {
        return executionContextStore.getStore()?.currentNext || null;
    }

    private get queryFilters(): ProcessedQueryFilters | undefined | null {
        return executionContextStore.getStore()?.queryFilters;
    }
    private set queryFilters(val: ProcessedQueryFilters | undefined | null) {
        const s = executionContextStore.getStore();
        if (s) s.queryFilters = val;
    }

    private get requestStartTime(): number {
        return executionContextStore.getStore()?.requestStartTime || 0;
    }
    private set requestStartTime(val: number) {
        const s = executionContextStore.getStore();
        if (s) s.requestStartTime = val;
    }
        ...
        while (currentPrototype && currentPrototype !== Object.prototype) {
            ...
            for (const prop of propertyNames) {
                ...
                if (prop !== 'constructor' && typeof (currentPrototype as any)[prop] === 'function') {
                    ...
                    if (currentPrototype === ControllerBase.prototype || currentPrototype.constructor.name === 'ControllerBase') {
    ...
    private handleResponseIfNeeded(result: any): void {
        ...
        if (result !== undefined && this.currentResponse && !this.isStreamResponse(result)) {
            ...
            throw new ProblematicResponseError(`No se preparó ninguna respuesta en un método de ${this.controllerName}`);
        }
    }
    ...
    private sendAutoResponse(result: any): void {
        ...
        const isUpdate = method === 'PATCH' || method === 'PUT';
        ...
        const response: ApiResponse = {
            success: true,
            message,
            ...(!isUpdate ? { data: processed.data } : {}),
            ...(processed.metadata && !isUpdate ? { metadata: processed.metadata } : {}),
        };
     ...
     */
    protected success<T = any>(data: T, message: string = 'Operación completada exitosamente', statusCode: number = 200): void {
        ...
        const isUpdate = method === 'PATCH' || method === 'PUT';
        const finalMessage =
            isUpdate && (message === 'Success' || message === 'Operación completada exitosamente' || message === 'Resource updated successfully') ? 'Datos actualizados correctamente' : message;
     ...
     */
    protected created<T = any>(data: T, message: string = 'Recurso creado correctamente'): void {
     ...
     */
    protected updated<T = any>(_data?: T, message: string = 'Datos actualizados correctamente'): void {
    ...
    protected setCookie(name: string, value: string, options: CookieOptions = {}): void {
        ...
        this.getResponse().cookie(name, value, {
            ...defaultOptions,
            ...options,
        });
    }
