...
export abstract class BaseDatabaseConnector implements IDatabaseConnector {
    ...
    getModel<T = any>(modelName: string): T {
        if (!this.models.has(modelName)) throw new Error(`Model '${modelName}' not registered in DB '${this.config.id}'`);
        ...
        try {
            ...
            if (typeof import.meta.env !== 'undefined') {
                ...
                for (const pathModel in modelModules) {
                    ...
                    if (!modelDefinition.default || typeof modelDefinition.default !== 'function') throw new Error(`Model ${modelFileName} does not export a valid class`);
                ...
                for (const modelName of modelNames) {
                    ...
                    if (!modelDefinition.default || typeof modelDefinition.default !== 'function') throw new Error(`Model ${modelName} does not export a valid class`);
