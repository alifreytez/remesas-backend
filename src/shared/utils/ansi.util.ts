...
export class ANSI {
    ...
    static link(url: string, { text, styles }: { text?: string; styles?: string | string[] } = {}): string {
        ...
        const formattedText = styles.length > 0 ? this.format(linkText, ...styles) : this.format(linkText, ...this.messageTypes.link);
     ...
     */
    static superLink(options: { url: string; text?: string; styles?: string[]; protocol?: 'http' | 'https' | 'ftp' | 'mailto' | 'file' | string }): string {
    ...
    static table(headers: string[], rows: string[][]): string {
        ...
        const headerLine = '│ ' + headers.map((header, i) => this.format(header.padEnd(colWidths[i]), 'bold', 'cyan')).join(' │ ') + ' │';
