...
export class Logger {
    ...
    private static formatMessage(type: LogTypes, message: string, { format, firm }: { format?: string; firm?: string } = {}): string {
    ...
    private static showLog(type: LogTypes, content: any[], { sepStart = false, sepEnd = false }: ISeparatorOptions = {}, allPrefixed: boolean = false) {
        const uniqueSep = '[_!_]';
        let _content: string | string[] = `${allPrefixed ? this.prefix : ''}${content.flat().join(uniqueSep + `\n${this.prefix}` + uniqueSep)}`;
    ...
    static info(message: string, { extension, firm, sepStart, sepEnd }: ILoggerOptions = {}): void {
        ...
        this.showLog('log', [this.formatMessage('info', `${message}`, { format: color, firm }), (extension ?? []) as Array<string>], { sepStart, sepEnd });
    }
    ...
    static error(message: string | null, error: Error, { sepStart, sepEnd }: ISeparatorOptions = {}): void {
        ...
        const traceId = (error as any)?.traceId ? `[traceId: ${color}${(error as any).traceId}${this.resetFormatCode}]` : '';
    ...
    static warn(message: string, { extension, firm, sepStart, sepEnd }: ILoggerOptions = {}): void {
        ...
        this.showLog('warn', [this.formatMessage('warn', `${message}`, { format: color, firm }), (extension ?? []) as Array<string>], { sepStart, sepEnd });
    }
