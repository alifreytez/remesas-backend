...
export const findMultipleParentFolders = (folderNames: string[], options: FindParentFolderOptions = {}): Record<string, string | null> => {
...
export const findParentFolderAsync = async (folderName: string, options: FindParentFolderOptions = {}): Promise<string | null> => {
    // Para una versión asíncrona real
    return Promise.resolve(findParentFolder(folderName, options));
};

export const findParentFolderDetailed = (folderName: string, options: FindParentFolderOptions = {}): FolderSearchResult => {
    const { startDir, includeSiblings = false, maxLevels = Infinity } = options;
