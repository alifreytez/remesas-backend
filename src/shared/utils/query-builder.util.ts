...
export class QueryBuilder {
    ...
    static processResponse(
        ...
        switch (responseType) {
            ...
            case 'array':
                if (filters) {
                    const metadata = this.buildPaginationMetadata(clonedData.length, filters.pagination.offset, filters.pagination.limit);
    ...
    private static deepClone(obj: any): any {
        ...
        if (typeof obj === 'object') {
            ...
            for (const key in obj) if (Object.prototype.hasOwnProperty.call(obj, key)) cloned[key] = this.deepClone(obj[key]);
    ...
    static validateOrderParams(order: ProcessedQueryFilters['order']): {
        ...
        for (const [field, direction] of order) {
            ...
            if (field && field.trim() && (direction === 'asc' || direction === 'desc')) valid.push([field.trim(), direction]);
