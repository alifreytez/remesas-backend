...
export class Validator {
    ...
    static isObjectId(value: string | number): boolean {
        if (value === undefined || value === null || value === '') return false;
        const str = String(value).trim();
        // Compatible con IDs enteros de bases de datos SQL (/^\d+$/), UUIDs y ObjectIds de MongoDB
        return /^\d+$/.test(str) || /^[0-9a-fA-F]{24}$/.test(str) || /^[0-9a-fA-F-]{36}$/.test(str);
    }

    static isValidId(value: string | number): boolean {
    ...
    static requireArg(obj: Record<string, unknown>, requiredArg: string, alarm: boolean = true): unknown | boolean | void {
