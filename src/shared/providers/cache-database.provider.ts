...
export class CacheDatabaseProvider {
    ...
    private constructor() {
        ...
        this._client.on('connect', () => {
            Logger.natural(ANSI.success(`[+] Connected to Redis at ${config.cacheDatabase.host}:${config.cacheDatabase.port}`));
        });
    ...
    static getInstance(): CacheDatabaseProvider {
        if (AppConfig.isProduction()) {
            ...
            if (!globalWithRedis[CACHE_DATABASE_CLIENT_SYMBOL]) globalWithRedis[CACHE_DATABASE_CLIENT_SYMBOL] = new CacheDatabaseProvider();
